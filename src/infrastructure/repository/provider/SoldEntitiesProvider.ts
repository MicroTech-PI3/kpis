...
export default class SoldEntitiesProvider implements ISoldEntitiesProvider {
  ...
  async findAllCategories(): Promise<SoldCategory[]> {
    return new Promise((resolve, reject) => {
      this.mySqlDBC
        .query<DBSoldEntity>(
          `SELECT C.NAME, (SUM(SIP.QUANTITY) / (SELECT SUM(SIP2.QUANTITY) FROM CATEGORY C2 JOIN PRODUCT P2 ON P2.CATEGORY_ID = C2.ID JOIN SOLD_ITEMS_has_PRODUCT SIP2 ON P2.ID = SIP2.PRODUCT_ID)) * 100 AS QUANTITY FROM CATEGORY C JOIN PRODUCT P ON P.CATEGORY_ID = C.ID JOIN SOLD_ITEMS_has_PRODUCT SIP ON P.ID = SIP.PRODUCT_ID GROUP BY C.ID ORDER BY QUANTITY DESC;`
        )
        .then((result) => {
          resolve(
